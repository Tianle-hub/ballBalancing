...
int main(int argc, char **argv)
{
    ...
    auto kalman_filter_cv  = BallControl::KalmanFilterCV();

    kalman_filter_cv.gerParam();
    ...
    while (ros::ok())
    {
        ...
        if (!kalman_filter_cv.isInitialized())
        {
            Eigen::Matrix<double,4,1> x0 = Eigen::Matrix<double,4,1>::Zero();
            x0(0) = zx(0);
            x0(2) = zx(1);
            kalman_filter_cv.init(x0);
        } else
        {
            kalman_filter_cv.predict();
        }

        if (t - t_prev >= 0.033)
        {
            kalman_filter_cv.updatePos(zx);    
            ROS_INFO_STREAM("*****************************************************");
            t_prev = t;
        ...
        auto x = kalman_filter_cv.getPosVel();
